...
class DeleteQuery(WaylayBaseModel):
    ...
    force_stop: Annotated[
        StrictBool | None, Field(description="Force task stop before deletion")
    ] = None
...
def _list_query_alias_for(field_name: str) -> str:
    ...
    if field_name == "filter":
        return "filter"
    if field_name == "resource_metadata_expression":
        return "resourceMetadataExpression"
...
class ListQuery(WaylayBaseModel):
    ...
    resource_metadata_expression: Annotated[
        StrictStr | None,
        Field(
