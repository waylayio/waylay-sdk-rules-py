...
from ..models.graph_format import GraphFormat


def _copy_query_alias_for(field_name: str) -> str:
    if field_name == "new_name":
        return "newName"
    return field_name


class CopyQuery(WaylayBaseModel):
    """Model for `copy` query parameters."""

    new_name: Annotated[
        StrictStr, Field(description="New name for the copied template")
    ]

    model_config = ConfigDict(
        protected_namespaces=(),
        extra="allow",
        alias_generator=_copy_query_alias_for,
...
class GetQuery(WaylayBaseModel):
    ...
    format: Annotated[
        GraphFormat | None, Field(description="Format of the graph definition")
    ] = None
...
class ListQuery(WaylayBaseModel):
    ...
    hits: Annotated[
        StrictInt | None,
        Field(description="(Paging) maximal number of items returned"),
    ] = None
    ...
    id: Annotated[StrictStr | None, Field(description="filter on template name")] = (
        None
    )
    plugin: Annotated[
        StrictStr | None,
