...
from ..types.list_tasks_tags_key_stub import ListTasksTagsKeyStub
from ..types.task_created_stub import TaskCreatedStub
from ..types.task_entity_stub import TaskEntityStub
from ..types.task_specification_stub import TaskSpecificationStub
from ..types.task_with_runtime_info_stub import TaskWithRuntimeInfoStub
...
def _create_set_mock_response(httpx_mock: HTTPXMock, gateway_url: str):
    mock_response = TaskCreatedStub.create_json()
    httpx_mock_kwargs = {
        "method": "POST",
...
def _get_set_mock_response(httpx_mock: HTTPXMock, gateway_url: str, taskId: str):
    mock_response = TaskWithRuntimeInfoStub.create_json()
    httpx_mock_kwargs = {
        "method": "GET",
        "url": re.compile(f"^{gateway_url}/rules/v1/tasks/{taskId}(\\?.*)?"),
        "content": json.dumps(mock_response, default=str),
...
async def test_get(service: RulesService, gateway_url: str, httpx_mock: HTTPXMock):
    ...
    check_type(resp, TaskWithRuntimeInfo)
...
async def test_list(service: RulesService, gateway_url: str, httpx_mock: HTTPXMock):
    ...
    kwargs = {
        ...
        "query": ListQuery(
            ...
            tags_key=ListTasksTagsKeyStub.create_json(),
            tags=[""],
            finished_before=56,
    ...
    check_type(resp, list[TaskWithRuntimeInfo])
...
async def test_list_without_types(
    ...
    kwargs = {
        "query": {
            ...
            "tags.key": ListTasksTagsKeyStub.create_json(),
            "tags": [""],
            "finishedBefore": 56,
